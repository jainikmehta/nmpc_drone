...
from dynamics_v2_avoid_multiple_dynamic_obstacles_traj import unicycle_dynamics
...
class ref_generator_2d:
    ...
    def generate_waypoints(self, previous_waypoints, current_state, obstacle_centers_prediction):
        ...
        for i in range(self.pred_horizn):
            ...
            for j in range(obstacle_centers_prediction.shape[0]):  # Iterate over each obstacle
                # Calculate distance to the obstacle center
                dist_to_obstacle = np.linalg.norm(next_waypoint_pos - obstacle_centers_prediction[j][i])
                ...
                if dist_to_obstacle < self.collision_threshold:
                    ...
                    vec_obstacle_center_to_waypoint = next_waypoint_pos - obstacle_centers_prediction[j][i]
                    ...
                    if np.linalg.norm(vec_obstacle_center_to_waypoint) <= 1e-6:
                        vec_obstacle_center_to_prev_waypoint = current_state[:2] - obstacle_centers_prediction[j][i]
                        ...
                        vec_obstacle_center_to_prev_waypoint = next_waypoint_pos - obstacle_centers_prediction[j][i]
                        vec_perpendicular = np.array([-vec_obstacle_center_to_prev_waypoint[1], vec_obstacle_center_to_prev_waypoint[0]])
                        avoidance_direction = vec_perpendicular / np.linalg.norm(vec_perpendicular)
...
class nmpc_node:
    ...
    def __init__(self, num_states, num_controls, 
                 ...
                 num_obstacles, buffer_distance_from_obstacle_boundary, 
                 min_safe_dist_from_obstacle_center):
    ...
    def get_constraints(self,):
        ...
        for k in range(self.pred_horizn): # For each time step in the prediction horizon
            ...
            for obs_idx in range(self.num_obstacles): # For each obstacle
                obs_center = self.obstacle_centers_prediction[obs_idx][k] # Obstacle center at time step k
    ...
    def solve_nmpc(self, ref_waypoints, current_state, obstacle_centers_prediction):
        ...
        self.obstacle_centers_prediction = obstacle_centers_prediction
