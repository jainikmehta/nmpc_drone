...
from utils_v2_avoid_multiple_dynamic_obstacles_traj import nmpc_node, ref_generator_2d
from plotting_v2_avoid_multiple_dynamic_obstacles_traj import plot_states_controls
...
if random_obstacles == False:
    initial_obstacle_centers = np.array([[0.1, 0.75, 0.035], [0.9, 0.5, -0.12], [0.1, 0.25, 0.035]])  # Fixed obstacle centers with speed
    num_obstacles = len(initial_obstacle_centers)
else:
    initial_obstacle_centers =  np.random.rand(num_obstacles, 2) * 1.0
...
nmpc_node_robot = nmpc_node(num_states=num_states, 
...
while goal_reached == False:
    ...
    obstacle_centers_prediction = np.empty((len(initial_obstacle_centers), pred_horizn), dtype=object)
    for j in range(len(initial_obstacle_centers)):
        x0, y0, vx = initial_obstacle_centers[j][0], initial_obstacle_centers[j][1], initial_obstacle_centers[j][2]
        obstacle_centers_prediction[j][0] = (x0, y0)
        for i in range(1, pred_horizn):
            prev_x, prev_y = obstacle_centers_prediction[j][i-1]
            new_x = prev_x + vx
            obstacle_centers_prediction[j][i] = (new_x, y0)

    ref_waypoints = ref_generator.generate_waypoints(previous_waypoints=np.array(previous_waypoints),
                                                    current_state=current_state,
                                                    obstacle_centers_prediction=obstacle_centers_prediction)
    previous_waypoints = ref_waypoints
    print("Generated Waypoints:")
    print(ref_waypoints)

    # Solve NMPC
    opt_control, opt_states, min_h_values = nmpc_node_robot.solve_nmpc(ref_waypoints=ref_waypoints, 
                                                                       current_state=current_state,
                                                                       obstacle_centers_prediction=obstacle_centers_prediction)
    ...
    plot_states_controls(
        ...
        omega_max=omega_max,
        num_obstacles=num_obstacles,
        obstacle_centers=initial_obstacle_centers,  # current positions
        obstacle_centers_prediction=obstacle_centers_prediction,  # predicted positions
        buffer_distance_from_obstacle_boundary=buffer_distance_from_obstacle_boundary, 
        obstacle_radius=obstacle_radius,
        min_dist_from_obstacle_center=min_safe_dist_from_obstacle_center, 
        min_h_values=min_h_values,
        save_path="plots", step=store_opt_states.__len__() if store_opt_states is not None else 0
    )
    store_opt_states.append(opt_states)
    store_opt_controls.append(opt_control)

    initial_obstacle_centers[:, 0:2] = np.vstack(obstacle_centers_prediction[:, 1])  # Update obstacle centers for next iteration obstacle moves by one step